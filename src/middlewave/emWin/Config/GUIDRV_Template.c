...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
    ...
    st7789v_draw_point(x, y, PixelIndex);
}
...
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  ...
  uint16_t x_PixelIndex;
  st7789v_read_point(x, y, &x_PixelIndex);
  PixelIndex = x_PixelIndex;
...
#endif
	st7789v_fill(x0, y0, x1, y1, LCD_COLORINDEX);
}
...
static void _DrawBitLine16BPP(GUI_DEVICE * pDevice, int x, int y, U16 const GUI_UNI_PTR * p, int xsize) {
  ...
  for (;xsize > 0; xsize--, x++, p++) {
	...
	x_CMD = 0x2C;
	*(volatile uint16_t*)(UCGUI_LCD_CMD) = x_CMD;
	for(; xsize > 0; xsize--, x++, p++)
	{
		pixel = *p;
		*(volatile uint16_t*)(UCGUI_LCD_DATA) = pixel;
	}
}
